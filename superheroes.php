...
$nameAlias = filter_input(INPUT_GET, "query", FILTER_SANITIZE_STRING);
$result = "";
if (empty($nameAlias)) {
    $result = "<ul>" . "\n";
    ...
    foreach ($superheroes as $superhero) {
        if (strcasecmp($nameAlias, $superhero['name']) === 0 || strcasecmp($nameAlias, $superhero['alias']) === 0) {
            $result = "<h3>" . strtoupper($superhero['alias']) . "</h3>" . "\n";
            $result .= "<h4>A.K.A." . strtoupper($superhero['name']) . "</h4>" . "\n";
            $result .= "<p>" . $superhero['biography'] . "</p>" . "\n";
            break;
        }
    }

    if (empty($result)) {
        $result = "<h4 class=\"queryerror\">SUPERHERO NOT FOUND</h4>" . "\n";
    }
}
echo $result;
?>
